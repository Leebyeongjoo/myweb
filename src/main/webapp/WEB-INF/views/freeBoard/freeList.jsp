					...
					<table class="table table-bordered">
						...
						<tbody>
							<c:forEach var="vo" items="${boardList }">
								<tr>
									<td>${vo.bno }</td>
									<td><a href="freeDetail?bno=${vo.bno }"> <c:out
												value="${fn:replace(vo.title , '&lt;', '')}" />
									</a></td>
									<td>${vo.writer }</td>
							...
							<ul class="pagination pagination-sm">
								...
								<c:forEach var="num" begin="${pageVO.startPage }"
									...
									<li class="${num==pageVO.pageNum ? 'active' : '' }"><a
										href="freeList?pageNum=${num }&amount=${pageVO.amount}">${num}</a>
									</li>
								</c:forEach>
								<c:if test="${pageVO.next }">
									<li><a
										href="freeList?pageNum=${pageVO.endPage+1 }&amount=${pageVO.amount}">다음</a></li>
								</c:if>
